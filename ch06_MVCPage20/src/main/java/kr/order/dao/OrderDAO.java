...
public class OrderDAO {
	...
	public int getOrderCount(String keyfield, String keyword)throws Exception{
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM zorder JOIN (SELECT order_num, LISTAGG(item_name,',') WITHIN GROUP (ORDER BY item_name)"
                    + " item_name FROM zorder_detail GROUP BY order_num) USING(order_num) JOIN zmember USING(mem_num) " + sub_sql;

			//preparedSate,emt 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	public List<OrderVO> getListOrder(int start, int end, String keyfield, String keyword)throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM("+"SELECT * FROM zorder JOIN (SELECT order_num, LISTAGG(item_num,',') WITHIN GROUP"
        			+ " (ORDER BY item_name) item_name FROM zorder_detail GROUP BY order_num) USING(order_num) JOIN zmember USING(mem_num) " + sub_sql
        			+ " ORDER BY order_num DESC)a) WHERE rnum >=? AND rnum <=?";
        	
			
			//PrepardStatememnt 객체 생성
			pstmt = conn.prepareStatement(sql);
	...
	public int getOrderCountByMem_num(String keyfield, String keyword, long mem_num)throws Exception{
        ...
        try {
        	...
        	sql = "SELECT COUNT(*) FROM zorder JOIN (SELECT order_num, LISTAGG(item_num,',') WITHIN GROUP"
        			+ " (ORDER BY item_name) item_name FROM zorder_detail GROUP BY order_num) USING(order_num) WHERE mem_num= ? " + sub_sql;
	...
	public List<OrderVO> getListOrderByMem_num(int start, int end, String keyfield, String keyword, long mem_num)throws Exception{
	...
	public List<OrderDetailVO> getListOrderDetail(long order_num)throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM zorder_detail WHERE order_num=? ODER BY item_num DESC";
